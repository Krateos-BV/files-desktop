...
extension Enumerator {
    ...
    private func drainTrashDeletions(
        ...
        // `reportBatchUpdates`. Both halves must land in one job that does not suspend between them,
        // so that whoever is handed the finished batch cannot observe the database before the
        // acknowledgement has written the soft-deletes. Draining off the main actor left that window
        // open to any caller, and to the test harness in particular, which has no other way to get
        // behind the write. See XNT-174.
        Task { @MainActor in
